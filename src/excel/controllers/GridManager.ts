...
export class GridHeaderManager {
    ...
    constructor( 
        ...
        this.helper.loadedRows = Math.min(this.initialLoadCount, this.isInfiniteRow ? Infinity : totalRow);
        this.helper.loadedCols = Math.min(this.initialLoadCount, this.isInfiniteCol ? Infinity : totalCol);
        this.update(viewportWidth, viewportHeight, this.zoomIndex);
    }

    public update(viewportWidth: number, viewportHeight: number, zoomIndex: number): void {
    ...
    public getCellSize(type: 'horizontal' | 'vertical', index: number): number {
        if (type === 'horizontal') {
            ...
            return this.customRowHeights.get(index) || this.baseCellHeight * this.zoomIndex;
        }
    }
    ...
    private _getVisibleHeaderCells(
        ...
        const isHorizontal = type === 'horizontal';
        const cells = isHorizontal ? this.headerCellsHorizontal : this.headerCellsVertical;
    ...
    public findStartingIndex(type: 'horizontal' | 'vertical', scroll: number): number {
        const cells = type === 'horizontal' ? this.headerCellsHorizontal : this.headerCellsVertical;
        let low = 0;
    ...
    public getTotalContainerHeight(): number {
        return this.headerCellsVertical.slice(0, this.helper.loadedRows).reduce(
